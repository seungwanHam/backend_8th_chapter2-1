...
export class CartController {
  ...
  @ApiResponse({ status: 200, description: '장바구니 추가 완료', schema: { 
    example: { 
  ...
  @ApiResponse({ status: 400, description: '재고 부족', schema: {
    example: {
      statusCode: 400,
      message: '재고가 부족합니다.',
    }
  ...
  @ApiResponse({ status: 200, description: '장바구니 항목', schema: { 
    example: { 
      items: [
        { 
          cartItemId: 'cart-item-123',
  ...
  @ApiResponse({ status: 404, description: '장바구니 항목 없음', schema: {
    example: {
      statusCode: 404,
  ...
  @ApiResponse({ status: 200, description: '장바구니 수정 완료', schema: { 
    example: { 
      cartItemId: 'cart-item-123',
      quantity: 2,
      totalPrice: 20000
  ...
  @ApiParam({ name: 'cartItemId', description: '장바구니 항목 ID', example: 'cart-item-123' })
  @ApiQuery({ name: 'userId', required: true, description: '사용자 ID', example: 'user-123' })
  @ApiResponse({ status: 200, description: '장바구니 삭제 완료', schema: { 
    example: { 
